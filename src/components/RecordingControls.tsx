...
function Tooltip({ label, children, position = "top" }: { label: string; children: ReactNode; position?: "top" | "bottom" }) {
  ...
  return (
    <div className="relative group/tip">
      {children}
      <div
        className={`absolute ${pos} px-2.5 py-1 rounded-lg bg-gray-900/90 backdrop-blur-sm text-white text-[11px] font-medium whitespace-nowrap
...
/* ── Divider ── */
function Divider() {
  return <div className="w-px h-7 bg-gray-200/60 mx-0.5" />;
}
...
export default function RecordingControls({
  ...
  return (
    ...
    >
      <div
        className="glass-panel px-3 py-2.5 flex items-center gap-2 cursor-grab active:cursor-grabbing"
        ...
        {isRecording ? (
          <>
            ...
            <Tooltip label="Recording duration">
              <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-lg bg-red-50 border border-red-100">
                <div className="w-2.5 h-2.5 rounded-full bg-red-500 recording-pulse" />
                <span className="font-mono text-sm text-red-600 font-semibold min-w-[52px] tabular-nums">
            ...
            <Tooltip label={isPaused ? "Resume recording" : "Pause recording"}>
              <button
                ...
                className={`btn-icon !p-2 ${isPaused ? "!bg-amber-50 !border-amber-200 !text-amber-600" : ""}`}
              >
                {isPaused ? (
            ...
            <Tooltip label="Stop recording">
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-xl bg-red-600 px-4 py-2 text-[13px] font-semibold text-white
                           shadow-md shadow-red-600/20 transition-all duration-150 ease-out
          ...
          <>
            ...
            <Tooltip label={cameraEnabled ? "Camera on" : "Camera off"}>
              <div className="relative group">
                <button
                  ...
                  className={`btn-icon !p-2 ${cameraEnabled ? "!bg-green-50 !border-green-200 !text-green-600" : ""}`}
                >
                  {cameraEnabled ? (
            ...
            <Tooltip label={micEnabled ? "Mic on" : "Mic off"}>
              <div className="relative group">
                <button
                  onClick={onToggleMic}
                  className={`btn-icon !p-2 ${micEnabled ? "!bg-green-50 !border-green-200 !text-green-600" : ""}`}
                >
                  {micEnabled ? (
                    <svg className="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            {captionsSupported && (
              <Tooltip label={captionsEnabled ? "Captions on" : "Captions off"}>
                <button
                  ...
                  className={`btn-icon !p-2 ${captionsEnabled ? "!bg-green-50 !border-green-200 !text-green-600" : ""}`}
                >
                  <svg className="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            <Tooltip label={hasSelection ? "Area selected" : "Select area"}>
              <button
                ...
                className={`btn-icon !p-2
                  ${isSelectingArea ? "!bg-green-50 !border-green-200 !text-green-600" : ""}
                  ${hasSelection && !isSelectingArea ? "!border-green-400 !text-green-600 !bg-green-50/50" : ""}`}
              >
                <svg className="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            <Tooltip label={!hasSelection ? "Select an area first" : "Start recording"}>
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-xl bg-green-600 px-4 py-2 text-[13px] font-semibold text-white
...
function DragHandle() {
  return (
    <div className="flex flex-col items-center gap-[3px] px-1 opacity-25">
