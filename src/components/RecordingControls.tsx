      ...
      >
        ...
        {isRecording ? (
          ...
          <div className="flex items-center gap-2 px-3 py-2">
            ...
            {/* ── Present ── */}
            <Tooltip label={frameCount > 0 ? `Present ${frameCount} slide${frameCount !== 1 ? "s" : ""}` : "Present — add frames to create slides"}>
              <button
                onClick={onStartPresentation}
                className={`relative inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-[12px] font-semibold transition-all duration-200 ease-out cursor-pointer
                  focus:outline-none focus:ring-2 focus:ring-amber-400/40
                  ${frameCount > 0
                    ? "bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 text-amber-700 hover:from-amber-100 hover:to-orange-100 hover:border-amber-300 hover:shadow-sm"
                    : "bg-white/80 border border-gray-200/80 text-gray-400 hover:bg-amber-50 hover:text-amber-600 hover:border-amber-200"
                  }`}
              >
                <svg className={iconCls} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 3v11.25A2.25 2.25 0 0 0 6 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0 1 18 16.5h-2.25m-7.5 0h7.5m-7.5 0-1 3m8.5-3 1 3m0 0 .5 1.5m-.5-1.5h-9.5m0 0-.5 1.5" />
                </svg>
                <span className="hidden sm:inline leading-none">Present</span>
                {frameCount > 0 && (
